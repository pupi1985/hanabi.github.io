...
- For example, in a 3-player rainbow game:
  ...
  - Since slot 2 has more positive information, Bob should choose to play slot 2 as red 3 (instead of slot 1, the leftmost card).
...
- For example, in a 3-player game:
  ...
  - On Bob's next turn, he blind-plays his red 2 from slot 2.
...
- Subsequently, it is possible to perform **another** _Finesse_ on a player who has already been _Finessed_. In this case, then the second _Finesse_ is queued after the first. The player is expected to blind-play into the _Finesses_ in order, from earliest to latest.
  - The reason for this convention is that it allows _Layered Finesses_ to work properly. In other words, if Bob thinks that he is _Finessed_ twice, he also has to entertain the possibility that he is _Layered Finessed_ once.
...
- Here, Cathy is actually the one making the mistake. _Finesses_ are considered to be basic moves, so the idea is that you should trust that your teammates are not making a mistake. If Bob is discarding, then it means that Cathy also has the exact same copy of the card, and Cathy should immediately blind-play.
...
- However, there are two important exceptions:
  1. Players are allowed to **defer** playing into a _Finesse_ if they are giving a clue that causes a _Finesse_ (and no-one else on the team could have done it).
